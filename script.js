...
start.addEventListener('click', () => {
  document.querySelector('.grid-footer').style.display="flex"
  document.querySelector('.full-game-board').style.display="block"
...
function findSolvableBoard() {
  while(1) {
    arr.sort(function(a, b){return 0.5 - Math.random()});
    var totalInversion  = 0
    var positionOfEmptyCell = 16
    for(var i = 0; i < 15; i++) {
      if(arr[i] == 0) positionOfEmptyCell = i + 1
    ...
    var positionFromBottom = parseInt(positionOfEmptyCell / 4)
    if(positionOfEmptyCell % 4 > 0) positionFromBottom += 1
    positionFromBottom = 5 - positionFromBottom
    
    if(positionFromBottom % 2 != totalInversion % 2) {
      break
    }
  }
}
